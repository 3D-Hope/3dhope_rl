...
def get_reward(parsed_scene, **kwargs):
    ...
    tv_idx = next(
        (k for k, v in idx_to_labels.items() if "tv_stand" in v.lower()), None
    )
    if tv_idx is None:
...
def test_reward():
    ...
    assert (
        reward[0].item() == 1.0
    ), f"Expected 1.0 for TV present, got {reward[0].item()}"
    ...
    parsed_scene_no_tv = {
        ...
        .to(device),
        "object_indices": torch.tensor(
            [[8] + [21] * (max_objects - 1)], device=device, dtype=torch.long
        ),
        "is_empty": torch.tensor(
            [[False] + [True] * (max_objects - 1)], device=device, dtype=torch.bool
        ),
        "device": device,
    }
    ...
    print(
        "For comprehensive testing, run: python dynamic_constraint_rewards/test_all_dynamic_rewards.py"
    )
