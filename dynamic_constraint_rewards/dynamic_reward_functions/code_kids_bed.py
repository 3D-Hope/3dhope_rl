...
def test_reward(**kwargs):
    ...
    idx_to_labels = kwargs.get("idx_to_labels", {
        ...
        '11': 'kids_bed', '12': 'nightstand', '13': 'pendant_lamp',
        '14': 'shelf', '15': 'single_bed', '16': 'sofa', '17': 'stool',
        '18': 'table', '19': 'tv_stand', '20': 'wardrobe'
    })
    ...
    parsed_scene_1 = {
        ...
        "one_hot": F.one_hot(torch.tensor([[kids_bed_idx, armchair_idx] + [num_classes - 1] * (max_objects - 2)], 
    ...
    parsed_scene_2 = {
        ...
        "one_hot": F.one_hot(torch.tensor([[armchair_idx, armchair_idx] + [num_classes - 1] * (max_objects - 2)], 
                             dtype=torch.long), num_classes).float().to(device),
        "device": device
    }
    ...
    parsed_scene_3 = {
        ...
        "one_hot": F.one_hot(torch.tensor([[num_classes - 1] * max_objects], 
                             dtype=torch.long), num_classes).float().to(device),
        "device": device
    }
    ...
    batch_sizes = torch.cat([
        parsed_scene_1["sizes"],
        parsed_scene_2["sizes"],
        parsed_scene_3["sizes"]
    ], dim=0)
