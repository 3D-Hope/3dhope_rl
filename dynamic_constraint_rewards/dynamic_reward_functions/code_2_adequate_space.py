...
def get_reward(parsed_scene, **kwargs):
    ...
    labels_to_idx = {v: int(k) if isinstance(k, str) else k for k, v in idx_to_labels.items()}
    ...
    for b in range(B):
        ...
        has_kids_bed = (valid_indices == kids_bed_idx).any().item() if kids_bed_idx >= 0 else False
        ...
        for i in range(len(valid_indices)):
            ...
            dx = abs(kids_bed_pos[0] - other_pos[0]) - (kids_bed_size[0] + other_size[0])
            dy = abs(kids_bed_pos[1] - other_pos[1]) - (kids_bed_size[1] + other_size[1])
            dz = abs(kids_bed_pos[2] - other_pos[2]) - (kids_bed_size[2] + other_size[2])
            
            # Horizontal clearance (x-z plane)
            horizontal_clearance = torch.sqrt(torch.clamp(dx, min=0.0)**2 + torch.clamp(dz, min=0.0)**2)
            
            # Reward based on clearance
            if horizontal_clearance < min_clearance:
                # Too close, penalty
                clearance_reward = -1.0 * (min_clearance - horizontal_clearance) / min_clearance
            elif horizontal_clearance < ideal_clearance:
                # Acceptable but not ideal
                clearance_reward = (horizontal_clearance - min_clearance) / (ideal_clearance - min_clearance) * 0.5
            else:
                # Good clearance
        ...
        if floor_vertices is not None and len(floor_vertices) > 0:
            ...
            floor_verts = torch.tensor(floor_vertices, device=device, dtype=torch.float32)
            ...
            for i in range(len(floor_verts)):
                ...
                bed_pos_2d = torch.tensor([kids_bed_pos[0], kids_bed_pos[2]], device=device)
                ...
                if wall_len_sq > 1e-6:
                    t = torch.clamp(torch.sum((bed_pos_2d - v1_2d) * wall_vec) / wall_len_sq, 0.0, 1.0)
                    projection = v1_2d + t * wall_vec
                    dist = torch.norm(bed_pos_2d - projection) - max(kids_bed_size[0], kids_bed_size[2])
            ...
            if wall_distances:
                ...
                if min_wall_dist < min_clearance:
                    ...
                    # Acceptable distance
                    wall_reward = (min_wall_dist - min_clearance) / (ideal_clearance - min_clearance) * 1.0
                else:
...
def test_reward(**kwargs):
    ...
    parsed_scene_1 = {
        ...
        "one_hot": F.one_hot(torch.tensor([[kids_bed_idx, wardrobe_idx] + [empty_idx] * (max_objects - 2)], 
    ...
    parsed_scene_3 = {
        "positions": torch.tensor([
            [[2.0, 0.5, 2.0]] + 
            [[0.0, 0.0, 0.0]] * (max_objects - 1)
        ], device=device),
        "sizes": torch.tensor([
            [[0.5, 0.8, 0.3]] + 
            [[0.01, 0.01, 0.01]] * (max_objects - 1)
        ], device=device),
        "object_indices": torch.tensor([
        ...
        "orientations": torch.tensor([
            [[1.0, 0.0]] + 
            [[0.0, 0.0]] * (max_objects - 1)
        ], device=device),
        "one_hot": F.one_hot(torch.tensor([[wardrobe_idx] + [empty_idx] * (max_objects - 1)], 
                             dtype=torch.long), num_classes).float().to(device),
        "device": device
    }
    ...
    parsed_scene_4 = {
        ...
        "one_hot": F.one_hot(torch.tensor([[kids_bed_idx] + [empty_idx] * (max_objects - 1)], 
                             dtype=torch.long), num_classes).float().to(device),
        "device": device
    }
