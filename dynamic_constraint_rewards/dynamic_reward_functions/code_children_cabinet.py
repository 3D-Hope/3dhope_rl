...
def get_reward(parsed_scene, **kwargs):
    ...
    for label_key in ["children_cabinet", "children cabinet", "kids_cabinet", "kids cabinet"]:
...
def test_reward(**kwargs):
    ...
    parsed_scene_1 = {
        ...
        "one_hot": F.one_hot(torch.tensor([[children_cabinet_idx, kids_bed_idx, 0] + [empty_idx] * (max_objects - 3)], 
    ...
    assert reward_1.item() == 10.0, f"Scene with children_cabinet should have reward 10.0, got {reward_1.item()}"
    ...
    parsed_scene_2 = {
        ...
        "one_hot": F.one_hot(torch.tensor([[kids_bed_idx, 0] + [empty_idx] * (max_objects - 2)], 
                             dtype=torch.long), num_classes).float().to(device),
        "device": device
    }
    ...
    assert reward_2.item() == 0.0, f"Scene without children_cabinet should have reward 0.0, got {reward_2.item()}"
    ...
    parsed_scene_3 = {
        ...
        "one_hot": F.one_hot(torch.tensor([[empty_idx] * max_objects], 
                             dtype=torch.long), num_classes).float().to(device),
        "device": device
    }
    ...
    assert reward_3.item() == 0.0, f"Empty scene should have reward 0.0, got {reward_3.item()}"
