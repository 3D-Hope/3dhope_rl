...
def compute_non_penetration_reward(parsed_scene):
    ...
    penetration_depth = compute_aabb_penetration_depth(positions, sizes, positions, sizes)  # (B, N, N)
    ...
    masked_penetration = torch.where(mask, penetration_depth, torch.zeros_like(penetration_depth))
...
def test_non_penetration_reward():
    ...
    print(f"Scene 2 (complete overlap): {rewards[1].item():.4f} (should be very negative)")
    print(f"Scene 3 (ceiling lamp ignored): {rewards[2].item():.4f} (should be 0)")
    print(f"Scene 4 (partial overlap): {rewards[3].item():.4f} (should be moderately negative)")
    print(f"Scene 5 (large penetration): {rewards[4].item():.4f} (should be very negative)")
    ...
    assert rewards[4].item() < -0.5, "Large penetration should have very negative reward"
