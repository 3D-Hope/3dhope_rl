...
def test_composite_reward_system():
    ...
    scene1 = create_bedroom_scene([
        ...
        (12, [0.6, -0.98, -0.5], [0.0, 0.0, 0.0]),   # nightstand on ground, far right
        (20, [0.0, -0.98, 0.6], [0.0, 0.0, 0.0]),    # wardrobe on ground, back
        (10, [-0.6, -0.98, 0.6], [0.0, 0.0, 0.0]),   # dressing_table on ground
        (3, [0.0, 0.8, 0.0], [0.0, 0.0, 0.0]),       # ceiling_lamp (ignored in collision)
    ])
    ...
    print(f"  gravity: {reward_weights['gravity']} (raw range: -2.7 to 0 → weighted: -13.5 to 0)")
    print(f"  non_penetration: {reward_weights['non_penetration']} (raw range: -25 to 0 → weighted: -12.5 to 0)")
    print(f"  must_have: {reward_weights['must_have_furniture']} (raw range: -10 to 0 → weighted: -10 to 0)")
    print(f"  object_count: {reward_weights['object_count']} (raw range: -9 to -1 → weighted: -9 to -1)")
    ...
    for i, name in enumerate(scene_names):
        ...
        for component_name, component_values in components.items():
            ...
            print(f"    {component_name:20s}: {component_values[i].item():8.4f} (weight: {weight:.1f}, weighted: {component_values[i].item() * weight:8.4f})")
    ...
    for rank, idx in enumerate(sorted_indices):
        print(f"  {rank+1}. {scene_names[idx]:25s} (reward: {total_rewards[idx].item():8.4f})")
    
    # Assertions
    assert total_rewards[0] > total_rewards[1], "Good bedroom should beat missing bed"
    assert total_rewards[0] > total_rewards[2], "Good bedroom should beat floating furniture"
    assert total_rewards[0] > total_rewards[3], "Good bedroom should beat overlapping furniture"
