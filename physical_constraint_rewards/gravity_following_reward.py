import torch
import numpy as np
from physical_constraint_rewards.commons import idx_to_labels, ceiling_objects
...
def compute_gravity_following_reward(parsed_scene):
    ...
    ceiling_indices = [idx for idx, label in idx_to_labels.items() if label in ceiling_objects]
    ...
    masked_distances = torch.where(
        ...
        floor_distance,
        torch.zeros_like(floor_distance)
    )
    
    # Sum distances per scene
    total_distance = masked_distances.sum(dim=1)  # (B,)(using this as penalty: enalizes scenes with more objects more heavily, regardless of quality. so use avg)
    ...
    # reward = -avg_distance
    reward = -total_distance  # using total distance as penalty: penalizes scenes with more objects more heavily, regardless of quality.
...
def test_gravity_following_reward():
    ...
    def create_scene(object_types, positions_normalized, sizes_normalized):
        ...
        for i, (obj_type, pos, size) in enumerate(zip(object_types, positions_normalized, sizes_normalized)):
            # One-hot encoding
            ...
            scene[i, num_classes+3:num_classes+6] = torch.tensor(size, device=device)
            # Orientation (cos, sin)
    ...
    scene1 = create_scene(
        object_types=[4, 4, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21],  # 2 chairs, rest empty
    ...
    scene2 = create_scene(
        object_types=[4, 16, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21],  # chair, sofa, rest empty
        positions_normalized=[
    ...
    scene3 = create_scene(
        object_types=[3, 4, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21],  # ceiling_lamp, chair, rest empty
        positions_normalized=[
    ...
    print(f"\nResults:")
    print(f"Scene 1 (objects on ground): {rewards[0].item():.4f} (should be close to 0)")
    print(f"Scene 2 (floating objects): {rewards[1].item():.4f} (should be negative)")
    print(f"Scene 3 (ceiling lamp ignored): {rewards[2].item():.4f} (should be close to 0)")
    
    # Verify
    assert rewards[0] > rewards[1], "Scene with grounded objects should have higher reward than floating"
