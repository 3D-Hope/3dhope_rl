...
idx_to_labels = {0: 'armchair', 1: 'bookshelf', 2: 'cabinet', 3: 'ceiling_lamp', 4: 'chair', 5: 'children_cabinet', 6: 'coffee_table', 7: 'desk', 8: 'double_bed', 9: 'dressing_chair', 10: 'dressing_table', 11: 'kids_bed', 12: 'nightstand', 13: 'pendant_lamp', 14: 'shelf', 15: 'single_bed', 16: 'sofa', 17: 'stool', 18: 'table', 19: 'tv_stand', 20: 'wardrobe'}
...
def parse_and_descale_scenes(scenes, num_classes=22):
    ...
    orientations = scenes[:, :, num_classes+6:num_classes+8]  # [cos_theta, sin_theta]
...
def get_composite_reward(scenes, num_classes=22, reward_weights=None, importance_weights=None, **kwargs):
    """
    ...
    NORMALIZATION_CONFIG = {
        'gravity': {
            'type': 'tanh',
            'scale': 1.0,  # Sensitivity: penalty -1.0 → normalized -0.76 (considered "severe")
            # NOT a maximum! Can handle arbitrarily large penalties smoothly
        },
        'non_penetration': {
            'type': 'tanh',
            'scale': 5.0,  # Sensitivity: penalty -5.0 → normalized -0.76 (considered "severe")
    ...
    if 'gravity' in rewards_to_compute:
        from physical_constraint_rewards.gravity_following_reward import compute_gravity_following_reward
        reward_components['gravity'] = compute_gravity_following_reward(parsed_scene)
    
    if 'object_count' in rewards_to_compute:
        from physical_constraint_rewards.object_count_reward import compute_object_count_reward
        # Use NLL mode for per-scene credit assignment (not KL mode)
        reward_components['object_count'] = compute_object_count_reward(parsed_scene, mode='nll')
    
    if 'must_have_furniture' in rewards_to_compute:
        from physical_constraint_rewards.must_have_furniture_reward import compute_must_have_furniture_reward
        room_type = kwargs.get('room_type', 'bedroom')
        reward_components['must_have_furniture'] = compute_must_have_furniture_reward(parsed_scene, room_type=room_type)
    
    if 'non_penetration' in rewards_to_compute:
        from physical_constraint_rewards.non_penetration_reward import compute_non_penetration_reward
        reward_components['non_penetration'] = compute_non_penetration_reward(parsed_scene)
