...
def draw_2d_gaussian(center: Tuple[float, float], size: Tuple[float, float], 
                     angle: float, image_size: int = 256) -> np.ndarray:
    """
...
def find_shortest_path(matrix: np.ndarray, start: Tuple[int, int], 
...
def map_to_image_coordinate(point: Tuple[float, float], scale: float, 
...
def image_to_map_coordinate(point: Tuple[int, int], scale: float, 
...
def create_occupancy_map(positions: torch.Tensor, sizes: torch.Tensor, 
                        orientations: torch.Tensor, scene_center: torch.Tensor,
                        scale: float, image_size: int = 256) -> np.ndarray:
    """
    ...
    for pos, size, orient in zip(positions, sizes, orientations):
        ...
        size_px = (int(size[0].item() / scale * image_size / 2),
                   int(size[2].item() / scale * image_size / 2))
        angle = torch.atan2(orient[1], orient[0]).item()
        
        # Draw rotated rectangle
        box_points = cv2.boxPoints(((center[0], center[1]), size_px, -angle/np.pi*180))
