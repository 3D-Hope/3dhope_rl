...
def object_count_target_reward_function(scene_data, scene_vec_desc=None, cfg=None, target_count=8):
    """
    ...
    for i in range(batch_size):
        ...
        non_empty_mask = torch.argmax(class_labels[i, :, :num_classes], dim=1) != (num_classes - 1)
...
def double_bed_reward_function(scene_data, scene_vec_desc=None, cfg=None):
    ...
    double_bed_idx = next(idx for idx, name in class_to_name.items() if name == 'double_bed')
    nightstand_idx = next(idx for idx, name in class_to_name.items() if name == 'nightstand')
    ...
    for i in range(batch_size):
        ...
        if double_bed_count == 1:
            ...
            wall_threshold = 0.3
            near_wall_x = (torch.abs(bed_position[0] - room_bounds[0, 0]) < wall_threshold or
                          torch.abs(bed_position[0] - room_bounds[0, 1]) < wall_threshold)
            near_wall_z = (torch.abs(bed_position[2] - room_bounds[2, 0]) < wall_threshold or
                          torch.abs(bed_position[2] - room_bounds[2, 1]) < wall_threshold)
...
def bedroom_composition_reward_function(scene_data, scene_vec_desc=None, cfg=None):
    ...
    # Find indices of important objects
    bed_indices = [idx for idx, name in class_to_name.items() 
                 if 'bed' in name and name != 'dressing_table']
    nightstand_idx = next((idx for idx, name in class_to_name.items() 
                         if name == 'nightstand'), None)
    wardrobe_idx = next((idx for idx, name in class_to_name.items() 
                       if name == 'wardrobe'), None)
...
def custom_object_distribution_reward_function(scene_data, scene_vec_desc=None, cfg=None, expected_counts=None):
    """
